...
class App:
    def __init__(self, master):
        ...
        # 初始化消融实验开关变量
        self.ablation_vars = {
            'enable_gate': tk.BooleanVar(value=False),          # 门控输入调制（默认关闭）
            'enable_gradient_proxy': tk.BooleanVar(value=True), # 梯度代理约束
            'enable_quantization': tk.BooleanVar(value=False),  # 训练后量化映射
            'optimizer_type': tk.StringVar(value='Adam (默认)'), # 控制优化器类型
            'enable_seed': tk.BooleanVar(value=False),          # 随机种子开关（默认关闭）
            'seed_value': tk.IntVar(value=42),                  # 随机种子值
        }
    ...
    def create_ablation_panel(self, parent):
        ...
        # 优化器选择
        opt_frame = ttk.Frame(ablation_frame)
        opt_frame.pack(fill='x', pady=2)
        ttk.Label(opt_frame, text="核心优化算法:").pack(side='left', padx=(0, 5))
        opt_combo = ttk.Combobox(opt_frame, 
                                textvariable=self.ablation_vars['optimizer_type'],
                                values=['Adam (默认)', 'FOPI (针对非对称死区的分数阶优化)'],
        ...
        # 随机种子开关
        seed_frame = ttk.Frame(ablation_frame)
        seed_frame.pack(fill='x', pady=2)
        ttk.Checkbutton(seed_frame, text="固定随机种子（实验可复现）",
                       variable=self.ablation_vars['enable_seed'],
                       command=self._on_seed_toggle).pack(side='left')
        ttk.Label(seed_frame, text="种子值:").pack(side='left', padx=(15, 5))
        self.seed_spinbox = ttk.Spinbox(seed_frame, from_=0, to=999999,
                                        textvariable=self.ablation_vars['seed_value'],
                                        width=8, state='disabled')
        self.seed_spinbox.pack(side='left')
    ...
    def _on_seed_toggle(self):
        """处理随机种子开关切换"""
        enabled = self.ablation_vars['enable_seed'].get()
        self.seed_spinbox.config(state='normal' if enabled else 'disabled')
        status = "已启用" if enabled else "已禁用"
        self.append_log(f"随机种子{status}")
    ...
    def run_training(self, current_data_path, dataset_name, network_params, training_params):
        ...
        try:
            ...
            optimizer_type = self.ablation_vars['optimizer_type'].get()
            
            # 随机种子
            enable_seed = self.ablation_vars['enable_seed'].get()
            seed_value = self.ablation_vars['seed_value'].get() if enable_seed else None
            ...
            self.append_log(f"[消融] 门控: {'开' if enable_gate else '关'}, "
                          ...
                          f"优化算法: {optimizer_type}")
            if seed_value is not None:
                self.append_log(f"[种子] 随机种子已固定为: {seed_value}")
