...
class RecognitionDisplay(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        # 添加权重分布直方图按钮
        ttk.Button(viz_frame, text="显示权重分布直方图",
                   command=self.show_weight_distribution).pack(side='left', padx=5)
        # 添加加载预训练模型按钮
        ttk.Button(viz_frame, text="加载预训练模型",
                   command=self.load_pretrained_model).pack(side='left', padx=5)
    ...
    def show_weight_distribution(self):
        ...
        try:
            # 检查是否有权重矩阵数据
            if not hasattr(self, 'weight_matrices') or not self.weight_matrices:
                # 尝试从training_manager获取
                if hasattr(self, 'training_manager'):
                    visualization_data = self.training_manager.get_visualization_data()
                    self.weight_matrices = visualization_data['weight_matrices']
                else:
                    messagebox.showerror("错误", "没有可用的权重数据，请先训练模型")
                    return

            if self.weight_matrices is not None:
                # 导入WeightDistributionVisualizer类
                from weight_distribution_visualizer import WeightDistributionVisualizer
                
                # 创建可视化器实例
                visualizer = WeightDistributionVisualizer()
                
                # 使用最新的权重矩阵进行可视化
                visualizer.visualize_from_weights(self.weight_matrices)
            else:
                messagebox.showerror("错误", "权重数据为空，请先训练模型")
        except Exception as e:
            messagebox.showerror("错误", f"显示权重分布直方图失败: {str(e)}")
    ...
    def load_pretrained_model(self):
        ...
        try:
            ...
            model_path = filedialog.askopenfilename(
                title="选择预训练模型文件",
                filetypes=[("PyTorch模型文件", "*.pth")]
            )

            if not model_path:
                return  # 用户取消选择

            # 使用 WeightDistributionVisualizer 加载并可视化模型
            from weight_distribution_visualizer import WeightDistributionVisualizer
            visualizer = WeightDistributionVisualizer()
            visualizer.visualize_from_model(model_path)

            self.update_result_text(f"成功加载预训练模型: {model_path}")

        except Exception as e:
            error_msg = f"加载预训练模型失败: {str(e)}"
            messagebox.showerror("错误", error_msg)
            self.update_result_text(error_msg)
            print(f"Error details: {str(e)}")
