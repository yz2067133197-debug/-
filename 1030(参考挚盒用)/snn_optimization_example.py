...
# 在文件开头导入STDPTrainingConfig
from training_manager import TrainingManager, STDPTrainingConfig
import torch

# 在demonstrate_snn_optimization函数中修改SNN参数获取方式
def demonstrate_snn_optimization():
    ...
    # 5. 从STDPTrainingConfig获取SNN参数，确保与训练配置一致
    config = STDPTrainingConfig()  # 使用默认参数
    snn_params = {
        'tau': config.tau,
        'v_threshold': config.v_threshold,
        'time_steps': config.time_steps
    }
    print(f"\n5. 使用统一的SNN参数: tau={snn_params['tau']}, v_threshold={snn_params['v_threshold']}, time_steps={snn_params['time_steps']}")
...
def compare_optimization_strategies():
    ...
    for strategy in strategies:
        ...
        n_params = {'tau': 20.0, 'v_threshold': 1.0}
        processor.normalize_data(num_points=100, snn_params=snn_params)
        
        # 修改后：
        # 从STDPTrainingConfig获取SNN参数，确保与其他策略一致
        ...
        print(f"  使用统一的SNN参数: tau={snn_params['tau']}, v_threshold={snn_params['v_threshold']}, time_steps={snn_params['time_steps']}")
        processor.normalize_data(num_points=100, snn_params=snn_params)
...
def integration_example():
    ...
    # 4. 从STDPTrainingConfig获取SNN参数，确保与训练配置一致
    config = STDPTrainingConfig()
    snn_params = {
        'tau': config.tau,
        'v_threshold': config.v_threshold,
    ...
    print(f"  - SNN参数已匹配: tau={snn_params['tau']}, v_threshold={snn_params['v_threshold']}, time_steps={snn_params['time_steps']}")
